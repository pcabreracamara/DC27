...
demo_transport_stream_id = 18 # demo value, an official value should be demanded to dvb org
demo_original_network_id = 8916 # demo value, an official value should be demanded to dvb org

demo_service_id = [101, 102]
...
for i in range(0, 2):
	...
	out.close
	os.system('/usr/bin/sec2ts ' + str(pmt_pid[i]) + ' < ./pmt'+str(i)+'.sec > ./pmt'+str(i)+'.ts')
	...
	out.close
	os.system('/usr/bin/sec2ts ' + str(ait_pid[i]) + ' < ./ait'+str(i)+'.sec > ./ait'+str(i)+'.ts')
